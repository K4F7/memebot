...
6. As an Archive Administrator, I want a Work to require at least one valid WorkMedia relationship before it is readable, so that QQ never exposes an empty or unassociated Work.
...
15. As an Archive Administrator, I want a Work with a valid WorkMedia relationship to become immediately readable, so that the MVP does not require a second publication workflow.
...
- Work and its WorkMedia relationships become visible to the read API once the relationship is complete and points to at least one non-withdrawn Media Item. There is no draft/published state in the MVP; unfinished or unassociated media must not appear as a Work in the read API. The read API does not require an R2 probe while listing a Work; a missing object returns not-found when its signed media URL is served.
- Physical deletion is not an MVP operation. Delete access is disabled for Work, Media, and WorkMedia. Any authenticated Archive Administrator may withdraw an erroneous Media Item with `withdrawnAt` while retaining its metadata, WorkMedia relationship, and private R2 object; the read API excludes withdrawn items. A Work with no remaining readable Media Items is hidden, while its stable Work identifier remains reserved.
