...
describe('root README plugin catalog', () => {
  it('documents exactly the five plugin packages', () => {
    ...
    const publishedPackages = readdirSync(pluginRoot, { withFileTypes: true })
      .filter((entry) => entry.isDirectory() && entry.name.startsWith('memebot-'))
      .map((entry) => JSON.parse(readFileSync(resolve(pluginRoot, entry.name, 'package.json'), 'utf8')).name)
      .sort()

    expect([...new Set(documentedPackages)].sort()).toEqual(expectedPackages)
    expect(publishedPackages).toEqual(expectedPackages)
  })
